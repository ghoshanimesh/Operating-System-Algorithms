...
class PriorityNonPE{
	...
	public static void main(String[] args) {
		...
		for (int i=0; i<no; i++) {
			...
			System.out.println("Enter the Process Id, Burst Time, Arrival Time and Priority");
			int pid = sc.nextInt();
			double burst = sc.nextDouble();
			double arr_time = sc.nextDouble();
			int priority = sc.nextInt();
			if (pid > -1 && burst > -1 && arr_time > -1 && priority > -1) {
				if (arr_time < min_at) {
					min_at = Math.min(min_at, (int)arr_time);
				}				
				tot_time += burst;
				al.add(new Process(pid,burst,arr_time,priority));				
			}else{
		...
		for (int i=0; i<no; i++) {
			System.out.print("P"+(i+1) + "\t" +al.get(i).getPid()+ "\t" + al.get(i).getBurst()+ "\t" + al.get(i).getArrivalTime() + "\t" + al.get(i).getPriority());
			System.out.println();
		}
		System.out.println();
