...
class Process{
	...
	public void setResRat(double res_rat){
		this.res_rat = res_rat;
	}
	...
	public double getResRat(){return this.res_rat;}	
}
class HRRN{
    ...
    public static void sortResRat(ArrayList<Process> alp, double length, int time){
        double rr = 0;
        for(int i = 0 ; i<length; i++){
            for(int j=0; j<length; j++){
            	rr = (time + alp.get(i).getBurst())/(alp.get(i).getBurst());
            	alp.get(i).setResRat(rr);
                if(alp.get(i).getResRat() > alp.get(j).getResRat()){
                    Collections.swap(alp, i, j);
                }
            }
        }
